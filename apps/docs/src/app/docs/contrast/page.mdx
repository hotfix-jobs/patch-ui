import { ContrastDemo, ContrastSummary } from "./contrast-demo";

export const metadata = { title: "Contrast", alternates: { canonical: "/docs/contrast" } };

<PageHeader
  title="Contrast"
  description="The automated readability contract for Patch UI’s text, surfaces, primary action, and semantic status colors."
/>
...
<CopyCommand command="npm run check:contrast -w packages/react" />
