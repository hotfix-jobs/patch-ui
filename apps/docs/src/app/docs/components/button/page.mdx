...
- **Outline variant:** Hairline-bordered button with `--patch-border` and a subtle `--patch-accent` hover fill (restrained). Distinct from `secondary` (no border, subtle fill).
