...
- **Focus:** the `focusRing` recipe (2px outline via `--focus-ring-color` + `--focus-ring-offset`, offset 2px outside the element border). Color reads `var(--primary)` — monochrome ink by default; a consumer's brand override colors every focused primitive. Inputs are the intentional exception: on focus their border shifts to `--primary` plus a soft low-alpha halo hugging the border (`--focus-halo`, 3px at 18% alpha) instead of the outside ring — text fields match `:focus-visible` on every click, so the treatment stays quieter than the button ring.
...
- **Interaction overlays:** transparent-at-rest controls (tertiary Button, Tab, Sidebar link, Toggle, Pagination, icon-only close/clear buttons) use `bg-layer-hover` for hover / focus-visible and `bg-layer-selected` for transient press (`active:`) and selected rows / nav items. Held states — a trigger while its popup is open (`data-[popup-open]`) and a Toggle switched on (`data-[pressed]`) — use `bg-hairline-strong`: the fill takes the hover border color (bordered variants set `border-transparent` so the alpha fill paints the border-box uniformly instead of double-stacking at the edge) and the control melts into a single solid chip, clearly distinct from merely hovered neighbors. When an alpha fill sits inside a border that stays visible (Button secondary `active:`), add `bg-clip-padding` so the fill stops at the border's inner edge — otherwise the alpha border composites over the alpha fill and renders darker than its token. These are alpha overlays (Radix grayA 3 / 5), not solid rungs, so the state reads on any container underneath.
